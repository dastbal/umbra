...
| [012](./ADR-012-arrow-key-selection-prompts.md) | Accepted — amended 3× 2026-08-26 | `cli`, `ux`, `tty`, `prompts`, `completion`, `hitl`, `approval`, `slash-commands` | Menus are navigated with the arrow keys through an own prompt engine on Node's keypress decoder; the typed prompts stay live as the non-TTY fallback, and the approval gate fails closed on Escape and Ctrl+C. **Amended:** a CLI change is not verified until `dist/` is rebuilt; slash commands come from one registry instead of four restatements; `prompts.ts` is the facade (`select` / `multiSelect` / `confirm` / `askText`) with the readline lifetime enforced in one place; and Tab completes a slash command from that same registry, while the live `/` palette stays deferred because it means replacing `readline` rather than configuring it. |
