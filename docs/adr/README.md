...
| [012](./ADR-012-arrow-key-selection-prompts.md) | Accepted — amended twice 2026-08-26 | `cli`, `ux`, `tty`, `prompts`, `hitl`, `approval`, `slash-commands` | Menus are navigated with the arrow keys through an own prompt engine on Node's keypress decoder; the typed prompts stay live as the non-TTY fallback, and the approval gate fails closed on Escape and Ctrl+C. **Amended:** a CLI change is not verified until `dist/` is rebuilt; slash commands come from one registry instead of four restatements; and `prompts.ts` is now the facade (`select` / `multiSelect` / `confirm` / `askText`) with the readline lifetime enforced in one place. |
