...
| [012](./ADR-012-arrow-key-selection-prompts.md) | Accepted — amended 4× 2026-08-26 | `cli`, `ux`, `tty`, `prompts`, `completion`, `line-editor`, `hitl`, `approval`, `slash-commands` | Menus are navigated with the arrow keys through an own prompt engine on Node's keypress decoder; the typed prompts stay live as the non-TTY fallback, and the approval gate fails closed on Escape and Ctrl+C. **Amended:** a CLI change is not verified until `dist/` is rebuilt; slash commands come from one registry instead of four restatements; `prompts.ts` is the facade (`select` / `multiSelect` / `confirm` / `askText`) with the readline lifetime enforced in one place; Tab completes from that same registry; and `line-editor.ts` now replaces `readline` at the chat prompt to show the live `/` palette, guarded by a `UMBRA_SIMPLE_PROMPT=1` escape hatch — which exposed that a lone ESC byte emits no keypress event without an owning `Interface`. |
